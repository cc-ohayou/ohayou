...
//----------------------------UPLOADFILES------------------------------------
    ...
    $.fn.ssi_uploader = function (opts) {
         // alert("lal"); 页面中有几处调用了这个地方 就会来访问几次 可通过alert看出
// 默认的一些上传选项可以在调用的时候进行覆盖，譬如最大文件尺寸有的地方需要大一点就调大 maxFileSize
        var defaults = {
            url: '',
            data: {},//发送请求的额外数据。例如$('input').ssi-uploader({url:'upload.php',data:{"name":"myDragon"}})
            locale: 'en',//使用本地化的语言
            preview: true,//有无预览
            dropZone: true,//带不带可拖拽区域
            maxNumberOfFiles: '',//最多可上传的文件数
          ...
          /*  responseValidation对象的可用属性有：
            ...
            maxFileSize: 5,//图片大小限制 默认单位mb 可改动 经测试可用
            ajaxOptions: {},//{type:'DELETE'}扩展默认的$.ajax函数的选项。
